...
function getQuery(params: getQueryParams) {
  return `Type.Object({
    ...
    ${params.scalarFields
      .filter(field => getTypeboxScalar(field.type) !== "Unknown")
      .map(
        (field) =>
...
function getBodyObject(selection: selectionType[]): string {
  const scalarFields = getScalarFields(selection);
  const enumFields = getEnumFields(selection);
  const objectFields = getObjectFields(selection);
  return `Type.Object({
    ${scalarFields
      .filter(field => getTypeboxScalar(field.type) !== "Unknown")
      .map(
...
export default function file(params: typeParams) {
  const hasEnumFields = getEnumFields(params.model.fields).length > 0;
  return `import { Type, Static } from "@sinclair/typebox";
  ${hasEnumFields ? 'import * as PrismaClient from "@prisma/client";' : ''}
